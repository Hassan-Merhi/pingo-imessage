...
Wave 3 added the first five complete board/strategy game engines and playable iMessage interfaces:
...
Wave 4 completes the launch catalog with five deterministic physics/precision games:

- 8-Ball — ball motion, collision/cushion approximation, pockets, solids/stripes, scratches and 8-ball win/loss logic
- Cup Pong — aim/arc throws, six-cup racks and sink detection
- Basketball — five-shot shootout using release angle and power
- Darts — 301 straight-out, three-dart visits, doubles/triples/bulls and bust handling
- Mini Golf — nine holes, obstacles, ball physics, strokes and total-score result

All ten games use the Wave 2 match envelope and revision guard. A player makes a move inside Messages, Pingo simulates or validates it locally, prepares the updated interactive message in the compose field, and the player sends that card back to the opponent.

Physics games are deterministic and asynchronous by design: aim/power input is simulated into canonical game state that travels in the Pingo card, so opponents do not need a continuous real-time connection to play.

Sea Battle is deliberately different because its ship positions are secret. A player's five-ship fleet is validated and stored only in that player's Messages-extension sandbox. Fleet coordinates never enter the shared iMessage payload; shared state contains only readiness, pending shots and public hit/miss/sunk results. A Sea Battle match therefore needs to continue on the device that holds that player's private fleet.
