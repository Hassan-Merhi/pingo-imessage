...
final class MiniRacingPhase5MessageFlowTests: XCTestCase {
    private let baseURL = URL(string: "https://pingo.example/messages")!

    func testSenderReceiverMiniRacingTurnExchangeRoundTrips() throws {
        let now = Date(timeIntervalSince1970: 1_787_200_000)
        let host = PingoPublicProfile(id: UUID(), username: "race_host", createdAt: now, updatedAt: now)
        let guest = PingoPublicProfile(id: UUID(), username: "race_guest", createdAt: now, updatedAt: now)
        let challenge = PingoMatchReducer.challenge(gameID: .miniRacing, creator: host, now: now)
        ...
        let first = try PingoExtraGameEngine.submit(move: .init(primary: 75, secondary: 50), to: current, actorID: host.id, expectedRevision: current.revision, now: now.addingTimeInterval(2))
        current = try roundTrip(PingoMessagePayload(action: .turn, sender: host, match: first)).match
        var state = try PingoExtraGameEngine.state(from: current.gameState, gameID: .miniRacing, matchID: current.id)
        XCTAssertEqual(state.attempts[0], 1)
        XCTAssertEqual(current.currentPlayerID, guest.id)

        let second = try PingoExtraGameEngine.submit(move: .init(primary: 75, secondary: 50), to: current, actorID: guest.id, expectedRevision: current.revision, now: now.addingTimeInterval(3))
        current = try roundTrip(PingoMessagePayload(action: .turn, sender: guest, match: second)).match
        state = try PingoExtraGameEngine.state(from: current.gameState, gameID: .miniRacing, matchID: current.id)
    ...
    func testMiniRacingTurnPayloadStaysInsideMessageURLBudget() throws {
        let host = PingoPublicProfile(username: "race_host")
        let guest = PingoPublicProfile(username: "race_guest")
        let challenge = PingoMatchReducer.challenge(gameID: .miniRacing, creator: host)
        let accepted = try PingoMatchReducer.accept(challenge, opponent: guest, expectedRevision: challenge.revision)
        let moved = try PingoExtraGameEngine.submit(move: .init(primary: 75, secondary: 50), to: accepted, actorID: host.id, expectedRevision: accepted.revision)
    ...
    func testStaleMiniRacingMessageCannotReplayOldTurn() throws {
        let host = PingoPublicProfile(username: "race_host")
        let guest = PingoPublicProfile(username: "race_guest")
        let challenge = PingoMatchReducer.challenge(gameID: .miniRacing, creator: host)
        let accepted = try PingoMatchReducer.accept(challenge, opponent: guest, expectedRevision: challenge.revision)
        let move = PingoExtraGameMove(primary: 75, secondary: 50)
