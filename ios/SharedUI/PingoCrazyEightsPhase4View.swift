...
struct PingoCrazyEightsPhase4View: View {
    ...
    private var localHandCount: Int { state.hands.indices.contains(player) ? state.hands[player].count : 0 }
    private var opponentHandCount: Int { state.hands.indices.contains(opponentIndex) ? state.hands[opponentIndex].count : 0 }
        ...
        ZStack {
            ...
            if match.status == .awaitingOpponent {
                centeredState(
                    ...
                    detail: opponent.map { "@\($0.displayName) can open the newest Pingo card to join Crazy Eights." } ?? "Your opponent can open the newest Pingo card to join Crazy Eights."
                )
                ...
                centeredState(
                    ...
                    detail: "Your cards stay locked until their play or draw is submitted."
                )
    ...
    private var statusRibbon: some View {
        if match.status == .active {
            HStack(spacing: 8) {
                ...
                Text(canMove ? "YOUR PLAY" : "OPPONENT’S PLAY")
                    .font(.system(size: 10, weight: .heavy, design: .rounded))
                    .tracking(0.9)
                Spacer(minLength: 6)
                Text("YOU \(localHandCount) • THEM \(opponentHandCount)")
    ...
    private var turnStrip: some View {
        HStack(spacing: 9) {
            Image(systemName: canMove ? "hand.draw.fill" : "hourglass")
                .font(.system(size: 12, weight: .bold))
            VStack(alignment: .leading, spacing: 1) {
                Text(turnTitle)
                    .font(.system(size: 10, weight: .heavy, design: .rounded))
                Text(canMove ? "Play a matching card or draw from the pile." : "Waiting for the opponent to finish their turn.")
    ...
    private var turnTitle: String {
        ...
        return canMove ? "PLAY YOUR HAND" : "OPPONENT THINKING"
    }
    ...
    private var resultState: some View {
        VStack(spacing: 10) {
            ...
            Image(systemName: localWon ? "trophy.fill" : "rectangle.stack.fill")
            ...
            Text(resultTitle)
                ...
                .foregroundStyle(.white)
            Text("Cards remaining: You \(localHandCount) • Opponent \(opponentHandCount)")
            ...
            Button(canContinueSeries ? "Next Game" : "Rematch", action: canContinueSeries ? onContinueSeries : onRematch)
                .buttonStyle(CrazyEightsPhase4PrimaryButtonStyle())
        }
    ...
    private var resultTitle: String {
        ...
        return localWon ? "Crazy Eights victory" : "Opponent cleared their hand"
    }
}

private struct CrazyEightsPhase4PrimaryButtonStyle: ButtonStyle {
