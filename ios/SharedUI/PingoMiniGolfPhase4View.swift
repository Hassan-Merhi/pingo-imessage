...
struct PingoMiniGolfPhase4View: View {
    ...
    private var holeNumber: Int {
        min(max(state.holeIndex + 1, 1), PingoMiniGolf.course.count)
    }

    private var localHoleStrokes: Int { value(state.holeStrokes, player) }
    private var opponentHoleStrokes: Int { value(state.holeStrokes, opponentIndex) }
    private var localTotal: Int { value(state.totals, player) + localHoleStrokes }
    private var opponentTotal: Int { value(state.totals, opponentIndex) + opponentHoleStrokes }
    ...
    var body: some View {
        ZStack {
            ...
            if match.status == .awaitingOpponent {
                centeredState(
                    symbol: "flag.fill",
                    eyebrow: "TEE TIME SET",
                    title: "Waiting for opponent",
                    detail: opponent.map { "@\($0.displayName) can open the newest Pingo card to join the course." } ?? "Your opponent can open the newest Pingo card to join the course."
                )
            } else if match.status == .active && !canMove {
                centeredState(
                    symbol: "figure.golf",
                    eyebrow: "BALL LOCKED",
                    title: "Opponent’s putt",
                    detail: "Your aim and flick controls stay locked until their stroke is complete."
                )
    ...
    private var statusRibbon: some View {
        if match.status == .active {
            HStack(spacing: 8) {
                ...
                Text(canMove ? "YOUR PUTT" : "OPPONENT’S PUTT")
                    .font(.system(size: 10, weight: .heavy, design: .rounded))
                    .tracking(0.9)

                Spacer(minLength: 6)

                Text("HOLE \(holeNumber)/\(PingoMiniGolf.course.count)")
    ...
    private var holeStrip: some View {
        HStack(spacing: 9) {
            Image(systemName: localHoled ? "checkmark.circle.fill" : "figure.golf")
            ...
            Text("YOU \(localTotal) • THEM \(opponentTotal)")
                .font(.system(size: 9, weight: .heavy, design: .monospaced))
                .opacity(0.66)
        }
    ...
    private var localHoled: Bool { boolValue(state.holed, player) }
    private var opponentHoled: Bool { boolValue(state.holed, opponentIndex) }

    private var holeTitle: String {
        if localHoled && opponentHoled { return "HOLE COMPLETE" }
        if localHoled { return "YOU’RE IN" }
        if state.lastAutoFinished { return "STROKE LIMIT REACHED" }
        return "HOLE \(holeNumber) • STROKE \(localHoleStrokes + 1)"
    }

    private var holeDetail: String {
        if localHoled && !opponentHoled { return "Opponent still needs to finish this hole." }
        if canMove { return "Aim on the green, then flick upward to putt." }
        return "Opponent is playing • their hole strokes: \(opponentHoleStrokes)."
    }
    ...
    private var resultState: some View {
        VStack(spacing: 10) {
            ...
            Text("Final strokes: You \(localTotal) • Opponent \(opponentTotal)")
            ...
            if canContinueSeries {
                Button("Next Game", action: onContinueSeries)
                    .buttonStyle(MiniGolfPhase4PrimaryButtonStyle())
            } else {
    ...
    private var resultTitle: String {
        ...
        guard match.winnerPlayerID != nil else { return "Course tied" }
        return localWon ? "Lowest score wins" : "Opponent took the course"
    }
    ...
    private func boolValue(_ values: [Bool], _ index: Int) -> Bool {
        values.indices.contains(index) ? values[index] : false
    }
}

private struct MiniGolfPhase4PrimaryButtonStyle: ButtonStyle {
