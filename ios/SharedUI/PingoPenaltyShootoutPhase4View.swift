...
struct PingoPenaltyShootoutPhase4View: View {
    ...
    var body: some View {
        ZStack {
            ...
            if match.status == .awaitingOpponent {
                ...
                centeredState(
                    symbol: "hourglass",
                    eyebrow: "PITCH LOCKED",
                    title: "Opponent’s kick",
                    detail: "Your aim and flick controls stay locked until their penalty is complete."
                )
        ...
        .confirmationDialog(
            "Resign this Penalty Shootout match?",
    ...
    private var statusRibbon: some View {
        if match.status == .active {
            HStack(spacing: 8) {
                ...
                Text(canMove ? "YOUR KICK" : "OPPONENT’S KICK")
                    .font(.system(size: 10, weight: .heavy, design: .rounded))
                    .tracking(0.9)

                Spacer(minLength: 6)

                Text("KICK \(min(5, attempts(player) + 1))/5")
    ...
    private var kickStrip: some View {
        HStack(spacing: 9) {
            Image(systemName: state.lastScore > 0 ? "soccerball.circle.fill" : "soccerball")
    ...
    private var kickTitle: String {
        ...
        if state.lastScore > 0 { return "GOAL" }
        return "KICK \(min(5, attempts(player) + 1))"
    }

    private var kickDetail: String {
        if canMove { return "Pick your spot, then flick upward to shoot." }
        return "Opponent is shooting • \(attempts(opponentIndex))/5 kicks taken."
    }
    ...
    private var resultState: some View {
        VStack(spacing: 10) {
            ...
            ZStack {
                ...
                Image(systemName: localWon ? "trophy.fill" : "soccerball")
            ...
            if canContinueSeries {
                Button("Next Game", action: onContinueSeries)
                    .buttonStyle(PenaltyShootoutPhase4PrimaryButtonStyle())
            } else {
    ...
    private var resultTitle: String {
        ...
        guard match.winnerPlayerID != nil else { return "Shootout tied" }
        return localWon ? "Shootout victory" : "Opponent won the shootout"
    }
...
private struct PenaltyShootoutPhase4PrimaryButtonStyle: ButtonStyle {
