...
struct PingoArcheryPhase4View: View {
        ...
        ZStack {
            ...
            if match.status == .awaitingOpponent {
                centeredState(
                    ...
                    detail: opponent.map { "@\($0.displayName) can open the newest Pingo card to join the range." } ?? "Your opponent can open the newest Pingo card to join the range."
                )
            } else if match.status == .active && !canMove {
                centeredState(
                    symbol: "hourglass",
                    eyebrow: "RANGE LOCKED",
                    title: "Opponent’s arrow",
                    detail: "Your target controls stay locked until their shot is complete."
                )
    ...
    private var statusRibbon: some View {
        if match.status == .active {
            HStack(spacing: 8) {
                ...
                Text(canMove ? "YOUR ARROW" : "OPPONENT’S ARROW")
                    .font(.system(size: 10, weight: .heavy, design: .rounded))
                    .tracking(0.9)

                Spacer(minLength: 6)

                Text("ARROW \(min(5, attempts(player) + 1))/5")
    ...
    private var arrowStrip: some View {
        HStack(spacing: 9) {
            Image(systemName: state.lastScore >= 9 ? "scope" : "arrow.up.right")
    ...
    private var arrowTitle: String {
        if state.lastScore >= 10 { return "BULLSEYE" }
        if !state.lastSummary.isEmpty { return state.lastSummary.uppercased() }
        return "ARROW \(min(5, attempts(player) + 1))"
    }

    private var arrowDetail: String {
        if canMove { return "Drag to aim, then flick upward to release." }
        return "Opponent is shooting • \(attempts(opponentIndex))/5 arrows played."
    }
        ...
        VStack(spacing: 10) {
            ...
            if canContinueSeries {
                Button("Next Game", action: onContinueSeries)
                    .buttonStyle(ArcheryPhase4PrimaryButtonStyle())
            } else {
    ...
    private var resultTitle: String {
        ...
        return localWon ? "Range victory" : "Opponent took the match"
    }
...
private struct ArcheryPhase4PrimaryButtonStyle: ButtonStyle {
