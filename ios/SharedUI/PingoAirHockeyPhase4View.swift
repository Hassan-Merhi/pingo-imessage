...
struct PingoAirHockeyPhase4View: View {
        ...
        ZStack {
            ...
            if match.status == .awaitingOpponent {
                centeredState(
                    ...
                    detail: opponent.map { "@\($0.displayName) can open the newest Pingo card to join the table." } ?? "Your opponent can open the newest Pingo card to join the table."
                )
                ...
                centeredState(
                    ...
                    detail: "Your mallet and flick controls stay locked until their shot is complete."
                )
    ...
    private var shotTitle: String {
        if state.lastScore == 1 { return "GOAL" }
        if !state.lastSummary.isEmpty { return state.lastSummary.uppercased() }
        return "SHOT \(min(7, attempts(player) + 1))"
    }

    private var shotDetail: String {
        if canMove { return "Set your lane, then flick right to shoot." }
        return "Opponent is shooting • \(attempts(opponentIndex))/7 shots played."
    }
    ...
    private var resultState: some View {
        VStack(spacing: 10) {
            ...
            ZStack {
                ...
                Image(systemName: localWon ? "trophy.fill" : "circle.hexagongrid.fill")
            ...
            if canContinueSeries {
                Button("Next Game", action: onContinueSeries)
                    .buttonStyle(AirHockeyPhase4PrimaryButtonStyle())
            } else {
    ...
    private var resultTitle: String {
        ...
        return localWon ? "Table victory" : "Opponent took the match"
    }
...
private struct AirHockeyPhase4PrimaryButtonStyle: ButtonStyle {
