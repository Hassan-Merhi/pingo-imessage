...
struct PingoWordHuntPhase4View: View {
    ...
    var body: some View {
        ZStack {
            ...
            if match.status == .awaitingOpponent {
                centeredState(symbol: "textformat.abc", eyebrow: "BOARD READY", title: "Waiting for opponent", detail: opponent.map { "@\($0.displayName) can open the newest Pingo card to join the hunt." } ?? "Your opponent can open the newest Pingo card to join the hunt.")
    ...
    private var statusRibbon: some View {
        if match.status == .active {
            HStack(spacing: 8) {
                Circle().fill(canMove ? Color.green.opacity(0.92) : Color.white.opacity(0.34)).frame(width: 7, height: 7)
                Text(canMove ? "YOUR WORD" : "OPPONENT’S WORD")
                ...
                Button { showResignConfirmation = true } label: {
                    Image(systemName: "flag.fill").font(.system(size: 10, weight: .bold)).frame(width: 28, height: 24)
                }
                .buttonStyle(.plain)
    ...
    private var turnStrip: some View {
        HStack(spacing: 9) {
            Image(systemName: canMove ? "hand.draw.fill" : "hourglass").font(.system(size: 12, weight: .bold))
            VStack(alignment: .leading, spacing: 1) {
                Text(turnTitle).font(.system(size: 10, weight: .heavy, design: .rounded))
                Text(turnDetail).font(.system(size: 9, weight: .semibold, design: .rounded)).opacity(0.68)
            }
            Spacer(minLength: 6)
    ...
    private var turnTitle: String {
        ...
        if let lastWord = state.usedWords.last { return lastWord.uppercased() }
        return canMove ? "TRACE A WORD" : "OPPONENT SEARCHING"
    }

    private var turnDetail: String {
        canMove ? "Trace adjacent letters, then send a valid unused word." : "Waiting for the opponent to finish their word."
    }

    private func centeredState(symbol: String, eyebrow: String, title: String, detail: String) -> some View {
        VStack(spacing: 8) {
            Image(systemName: symbol).font(.system(size: 21, weight: .bold)).foregroundStyle(.white.opacity(0.88))
            Text(eyebrow).font(.system(size: 9, weight: .heavy, design: .rounded)).tracking(1.2).foregroundStyle(.white.opacity(0.52))
            Text(title).font(.system(size: 19, weight: .heavy, design: .rounded)).foregroundStyle(.white)
            Text(detail).font(.system(size: 11, weight: .semibold, design: .rounded)).foregroundStyle(.white.opacity(0.64)).multilineTextAlignment(.center).frame(maxWidth: 250)
        }
    ...
    private var resultState: some View {
        VStack(spacing: 10) {
            Text(match.status == .resigned ? "MATCH ENDED" : (match.winnerPlayerID == nil ? "DRAW" : (localWon ? "YOU WIN" : "YOU LOSE")))
                .font(.system(size: 10, weight: .heavy, design: .rounded)).tracking(1.4).foregroundStyle(.white.opacity(0.56))
            ZStack {
                Circle().fill(localWon ? Color.green.opacity(0.18) : Color.white.opacity(0.08)).frame(width: 54, height: 54)
                Image(systemName: localWon ? "trophy.fill" : "textformat.abc").font(.system(size: 25, weight: .bold)).foregroundStyle(.white)
            }
            Text(resultTitle).font(.system(size: 22, weight: .heavy, design: .rounded)).foregroundStyle(.white)
            Text("Final score: You \(score(player)) • Opponent \(score(opponentIndex))")
                .font(.system(size: 10, weight: .bold, design: .rounded)).foregroundStyle(.white.opacity(0.54))
            if let series = match.series {
                Text("Series \(series.scoreText)").font(.system(size: 11, weight: .bold, design: .rounded)).foregroundStyle(.white.opacity(0.56))
            }
            if canContinueSeries {
                Button("Next Game", action: onContinueSeries).buttonStyle(WordHuntPhase4PrimaryButtonStyle())
            } else {
                Button("Rematch", action: onRematch).buttonStyle(WordHuntPhase4PrimaryButtonStyle())
            }
        }
