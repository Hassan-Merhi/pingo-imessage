...
struct PingoTriviaPhase4View: View {
    ...
    var body: some View {
        ZStack {
            PingoTriviaPhase3View(
                state: state,
                player: player,
                canMove: canMove && match.status == .active,
                onMove: { move in
                    PingoTriviaFeedback.answerSubmitted()
                    onMove(move)
                }
            )
            ...
            if match.status == .awaitingOpponent {
                centeredState(
                    ...
                    detail: opponent.map { "@\($0.displayName) can open the newest Pingo card to join the Trivia match." } ?? "Your opponent can open the newest Pingo card to join the Trivia match."
                )
            } else if match.status == .active && !canMove {
                centeredState(
                    symbol: "hourglass",
                    eyebrow: "QUESTION LOCKED",
                    title: "Opponent’s turn",
                    detail: "Your answers stay locked until the opponent submits their choice."
                )
        ...
        .onAppear {
            PingoTriviaFeedback.prepare()
        }
        .onChange(of: state.lastScore) { newScore in
            guard state.attempts.reduce(0, +) > 0 else { return }
            PingoTriviaFeedback.answerResolved(points: newScore)
        }
        .onChange(of: canMove) { isReady in
            if isReady && match.status == .active {
                PingoTriviaFeedback.turnReady()
            }
        }
        .onChange(of: match.status) { status in
            if status == .completed || status == .resigned {
                PingoTriviaFeedback.matchFinished(won: localWon)
            }
        }
        .confirmationDialog("Resign this Trivia match?", isPresented: $showResignConfirmation, titleVisibility: .visible) {
    ...
    private var statusRibbon: some View {
        if match.status == .active {
            HStack(spacing: 8) {
                ...
                Text(canMove ? "YOUR QUESTION" : "OPPONENT’S QUESTION")
                    .font(.system(size: 10, weight: .heavy, design: .rounded))
                    .tracking(0.9)
                Spacer(minLength: 6)
                Text("QUESTION \(state.challengeIndex + 1)")
    ...
    private var turnTitle: String {
        ...
        return canMove ? "CHOOSE YOUR ANSWER" : "OPPONENT THINKING"
    }

    private var turnDetail: String {
        canMove ? "Pick one answer and lock it in." : "Waiting for the opponent to answer their question."
    }
    ...
    private var resultState: some View {
        VStack(spacing: 10) {
            ...
            ZStack {
                ...
                Image(systemName: localWon ? "trophy.fill" : "questionmark.bubble.fill")
            ...
            if canContinueSeries {
                Button("Next Game", action: onContinueSeries)
                    .buttonStyle(TriviaPhase4PrimaryButtonStyle())
            } else {
    ...
    private var resultTitle: String {
        ...
        return localWon ? "Trivia victory" : "Opponent won the quiz"
    }

    private func score(_ index: Int) -> Int { state.scores.indices.contains(index) ? state.scores[index] : 0 }
}

private struct TriviaPhase4PrimaryButtonStyle: ButtonStyle {
