...
enum PingoMessagePreviewRenderer {
    ...
    private static func drawEightBallCard(in context: CGContext, size: CGSize, payload: PingoMessagePayload) {
        ...
        for ball in visibleBalls {
            drawPoolBall(ball.id, at: poolPoint(ball.position, in: feltRect), in: context)
        }

        if payload.match.status == .active,
           let cue = visibleBalls.first(where: { $0.id == 0 }) {
            ...
            context.move(to: cuePoint)
            context.addLine(to: CGPoint(x: min(feltRect.maxX - 8, cuePoint.x + 110), y: cuePoint.y))
            context.strokePath()
            context.restoreGState()
        }
    ...
    private static func poolPoint(_ point: PingoVector2, in feltRect: CGRect) -> CGPoint {
        CGPoint(
            x: feltRect.minX + feltRect.width * CGFloat(point.x),
            y: feltRect.minY + feltRect.height * CGFloat(point.y)
        )
    }
    ...
    private static func drawPoolBall(_ id: Int, at center: CGPoint, in context: CGContext) {
        ...
        context.saveGState()
        context.setShadow(offset: CGSize(width: 1.5, height: 2), blur: 2.5, color: UIColor.black.withAlphaComponent(0.36).cgColor)
        color.setFill()
        context.fillEllipse(in: rect)
        context.restoreGState()

        if id >= 9 {
            let stripe = CGRect(x: rect.minX + 1, y: rect.midY - 3.1, width: rect.width - 2, height: 6.2)
            UIColor.white.setFill()
            UIBezierPath(roundedRect: stripe, cornerRadius: 3).fill()
        }

        if id != 0 {
            let badge = CGRect(x: center.x - 4.4, y: center.y - 4.4, width: 8.8, height: 8.8)
            (id == 8 ? UIColor.black : UIColor.white).setFill()
            context.fillEllipse(in: badge)
            let number = "\(id)" as NSString
            number.draw(
                in: CGRect(x: center.x - 4, y: center.y - 3.8, width: 8, height: 8),
                withAttributes: [
                    .font: UIFont.systemFont(ofSize: 5.2, weight: .heavy),
                    .foregroundColor: id == 8 ? UIColor.white : UIColor.black,
        ...
        context.fillEllipse(in: CGRect(x: rect.minX + 2.4, y: rect.minY + 2.2, width: 3.3, height: 3.3))
    }

    private static func drawEightBallStatusPill(
        payload: PingoMessagePayload,
        state: PingoEightBallState,
        in context: CGContext,
        tableRect: CGRect
    ) {
        let title: String
        switch payload.match.status {
        case .awaitingOpponent: title = "CHALLENGE"
        case .active: title = state.groups == [0, 0] ? "OPEN TABLE" : "8 BALL"
        case .completed: title = "MATCH COMPLETE"
        case .resigned: title = "MATCH ENDED"
        case .draft: title = "PREPARING"
        case .expired: title = "EXPIRED"
        }
        ...
        let pill = CGRect(
            x: tableRect.midX - (textSize.width + 28) / 2,
            y: tableRect.minY + 13,
            width: textSize.width + 28,
            height: 28
        )
        UIColor.black.withAlphaComponent(0.66).setFill()
        UIBezierPath(roundedRect: pill, cornerRadius: 14).fill()
        text.draw(at: CGPoint(x: pill.minX + 14, y: pill.minY + 6), withAttributes: attrs)
    }
    ...
    private static func poolBallColor(_ id: Int) -> UIColor {
        switch id {
        case 0: return .white
        case 1, 9: return .systemYellow
        case 2, 10: return .systemBlue
        case 3, 11: return .systemRed
        case 4, 12: return .systemPurple
        case 5, 13: return .systemOrange
        case 6, 14: return .systemGreen
        case 7, 15: return UIColor(red: 0.48, green: 0.10, blue: 0.10, alpha: 1)
        case 8: return .black
